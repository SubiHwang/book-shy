...
public class ExchangeController {
    ...
    @Operation(
            ...
            description = "사용자가 참여하고 있는 예약된 거래 약속 목록을 조회합니다. (진행 중 또는 확정 상태)",
            parameters = {
                    @Parameter(name = "page", description = "페이지 번호 (0부터 시작)", example = "0"),
                    @Parameter(name = "size", description = "페이지 크기", example = "10")
            },
            responses = {
                    @ApiResponse(responseCode = "200", description = "✅ 예정 거래 목록 조회 성공")
            }
    )
    @GetMapping("/promises")
    public ResponseEntity<List<ExchangePromiseDto>> getExchangePromises(
            @RequestHeader("X-User-Id") Long userId,
            @ParameterObject Pageable pageable
    ) {
        List<ExchangePromiseDto> result = exchangePromiseService.getExchangePromises(userId, pageable);

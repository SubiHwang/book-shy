...
export const fetchUserAllLibrary = async (userId: number): Promise<Library[]> => {
  try {
...
// 사용자의 공개 서재 목록 조회
export const fetchUserPublicLibrary = async (userId: number): Promise<Library[]> => {
  try {
    const response = await authAxiosInstance.get<Library[]>(`/library/public?userId=${userId}`);
    return response as unknown as Library[];
  } catch (error) {

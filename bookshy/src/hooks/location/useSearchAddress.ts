...
const useSearchAddress = (onAddressSelected: (data: AddressData) => void) => {
  ...
  // 주소로 좌표를 가져오는 함수
  const fetchCoordinatesByAddress = useCallback(
  ...
  const openAddressSearch = useCallback(async () => {
    ...
    if (!window.daum || !window.daum.Postcode) {
      toast.warn('주소검색 서비스를 불러오는 중입니다. 잠시 후 다시 시도해주세요.');
      return;
    }
    ...
    new window.daum.Postcode({
      oncomplete: async function (data: PostcodeData) {
        ...
        const shortAddress = [simplifiedRegion1, region2, region3].filter(Boolean).join(' ');
